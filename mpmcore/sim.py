...
class MPMCuttingSim:
    ...
    def __init__(self, cfg, cutting_mesh_pack, knife_pack, board_pack=None,
        ...
        # ----- Transparency pause after reaching bottom -----
        self._transparency_pause_active = False
        self._transparency_pause_start_time = None
        # Pause duration: vertical(4s) + horizontal(4s) + cut_parallel(3s) + return(2s) = 13s
        # Camera return happens DURING pause (last 2 seconds)
        self._transparency_pause_duration = 13.0  # 13 seconds (vertical 4s + horizontal 4s + cut_parallel 3s + return 2s)
        self._pause_end_time = None  # Track when pause ended (for camera transition continuation)
        self._paused_at_bottom = False
        self._pause_used = False  # Track if pause has been used (only first cut)
        self._prev_knife_down = 1  # Track previous state to detect transition
        # Camera transition for pause
        self._camera_transition_start_time = None
        self._camera_transition_duration = 1.0  # 1 second transition time
        self._camera_vertical_duration = 4.0  # 4 seconds for vertical view
        self._camera_horizontal_duration = 4.0  # 4 seconds for horizontal view
        self._camera_cut_parallel_duration = 3.0  # 3 seconds for cut-parallel view
        self._camera_return_duration = 2.0  # 2 seconds for return transition
        self._camera_phase = 0  # 0: vertical, 1: horizontal, 2: cut-parallel, 3: return
        self._camera_return_complete = False  # Track if camera return transition is complete
        self._camera_original_eye = None
        self._camera_original_lookat = None
        self._camera_original_up = None
        self._camera_target_eye = None
        self._camera_target_lookat = None
        self._camera_target_up = None
        self._camera_horizontal_eye = None
        self._camera_horizontal_lookat = None
        self._camera_horizontal_up = None
        self._camera_cut_parallel_eye = None
        self._camera_cut_parallel_lookat = None
        self._camera_cut_parallel_up = None
    ...
    def step(self, T):
        ...
        # Check if pause duration elapsed (before any updates)
        if self._transparency_pause_active:
            elapsed = time.time() - self._transparency_pause_start_time
            if elapsed >= self._transparency_pause_duration:
                # Pause finished - resume simulation (but camera transition continues)
                self._transparency_pause_active = False
                self._paused_at_bottom = False
                # Store pause end time for camera transition continuation
                if self._pause_end_time is None:
                    self._pause_end_time = time.time()
                # Store pause start time for camera transition calculation
                if self._camera_transition_start_time is None:
                    self._camera_transition_start_time = self._transparency_pause_start_time
                # Reset transition detection to prevent repeated pauses
                self._prev_knife_down = int(self.knife._down[None])
                print(f"[PAUSE] Transparency pause finished, resuming simulation (camera transition continues)")
                # Force knife to start rising
                self.knife.force_up()
                # Force color update to restore original colors
                self._force_full_recolor = 1
                # Note: Camera transition will continue until complete
            else:
                # During pause - skip physics updates but continue rendering
                self._frame += 1
                # Force color update to apply transparency
                self._force_full_recolor = 1
                return
        ...
        # Use previous frame's state to detect transition
        knife_down_before = self._prev_knife_down
        ...
        # Check if knife just reached bottom (after update)
        knife_y_after = float(self.knife.y[None])
        knife_stop_y = float(self.knife.stop_y)
        knife_down_after = int(self.knife._down[None])
        
        # Detect transition: was going down, now stopped (regardless of exact stop_y due to board collision)
        transition_detected = (knife_down_before == 1) and (knife_down_after == 0)
        
        # Debug logging (more frequent near transition)
        if self._frame % 100 == 0 or transition_detected:  # Log every 100 frames or on transition
            print(f"[DEBUG] Frame {self._frame}: knife_y={knife_y_after:.4f}, stop_y={knife_stop_y:.4f}, down_before={knife_down_before}, down_after={knife_down_after}, transition={transition_detected}")
        
        # Trigger pause on any transition from down to up (knife reached bottom or hit board)
        # Only pause on the first cut
        if transition_detected and not self._paused_at_bottom and not self._pause_used and self.viewer_enabled:
            # Check if this is the first cut
            is_first_cut = (not self.multi_cutting_enabled) or (self.current_cut_idx == 0)
            if is_first_cut:
                # Knife just stopped going down - start pause (only first cut)
                self._transparency_pause_active = True
                self._transparency_pause_start_time = time.time()
                self._paused_at_bottom = True
                self._pause_used = True
                print(f"[PAUSE] First cut - Knife stopped at Y={knife_y_after:.4f} (stop_y={knife_stop_y:.4f}), starting {self._transparency_pause_duration:.1f}-second pause")
                print(f"[PAUSE] Debug: down_before={knife_down_before}, down_after={knife_down_after}")
                # Skip physics updates for this frame
                self._frame += 1
                self._force_full_recolor = 1
                return
        
        # Update previous state
        self._prev_knife_down = knife_down_after
        ...
        if self._out_manager is not None:
            def _make_rec(sim, sim_time, frame_idx):
                ...
                # ---- Relative EEF Force (matches F/T sensor reading) ----
                # Reaction force on knife in cutting direction (sim Y axis)
                # F[1] = force on particles in sim-Y; knife reaction = -F[1]
                # Positive F_knife_cutting = material resists downward cut (pushes knife up)
                F_knife_cutting = -float(F[1])
                eef_cfg = sim.cfg.get("output", {}).get("logging", {})
                eef_baseline = float(eef_cfg.get("eef_baseline", 0.0))
                eef_scale = float(eef_cfg.get("eef_scale", 1.0))
                relative_eef = eef_baseline + eef_scale * F_knife_cutting
