...
"8-fruit SCALED single-cut started at $(Get-Date)" | Out-File -FilePath $mainLog -Encoding utf8
$t0 = Get-Date
foreach ($f in $fruits) {
    $cfg = "configs/fruits/$f.yaml"
    $tFruit = Get-Date
    "[$(Get-Date -Format HH:mm:ss)] [START] $f" | Add-Content -Path $mainLog -Encoding utf8
    & $py -u run.py --config $cfg --headless --run-sim --show-board --rebuild-sdf --log-dir "logs/single_cut/$f" *> "$logDir/$f.scaled.log"
    $rc = $LASTEXITCODE
...
"8-fruit SCALED finished at $(Get-Date) -- total ${totalMin}min" | Add-Content -Path $mainLog -Encoding utf8
